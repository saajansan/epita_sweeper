rule cybsecmalware_obfuscated : PDF 
{
    ...
    condition:
        $magic in (0..1024) and $header and $reg0 or (any of ($js_obfuscation, $string_encoding))
}
...
rule jscodeexploits_obfuscated : PDF
{
    ...
    condition:
        $magic in (0..1024) and all of ($attrib*) or (any of ($js_obfuscation))
}
...
rule js_hashbased_signatures_obfuscated : PDF
{
    ...
    condition:
        $magic in (0..1024) and all of ($s*) or (any of ($js_obfuscation, $array_obfuscation, $hex_encoding, $base64_encoding))
}

rule _home_kali_Desktop_malware_igotyou_obfuscated 
{
    meta:
    ...
    condition:
        uint16(0) == 0x5025 and filesize < 800KB and all of ($s*) or (any of ($string_encoding, $base64_encoding))
}
...
rule PDF_Embedded_Exe_Obfuscated : PDF
{
    ...
    condition:
        $header at 0 and $Launch_Action and $exe or (any of ($string_encoding, $base64_encoding))
}
...
rule js_wrong_version_obfuscated : PDF raw
{
    ...
    condition:
        $magic in (0..1024) and $js and not $ver or (any of ($string_encoding, $base64_encoding, $hex_encoding, $array_obfuscation))
}
...
rule invalid_trailer_structure_obfuscated : PDF raw
{
	...
	condition:
		$magic in (0..1024) and not $reg0 and not $reg1 or (any of ($string_encoding, $base64_encoding, $hex_encoding, $array_obfuscation))
}
